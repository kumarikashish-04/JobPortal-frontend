  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-[#1A2B3E]/95 backdrop-blur-none shadow-lg border-b border-[#3B82F6]/20"
          : "bg-transparent"
      }`}
    >
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-3">
                <motion.div 
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#1A2B3E] backdrop-blur-none border border-[#3B82F6]/30 shadow-lg"
                >
        ...
        {mobileOpen && (
          ...
          >
            <div className="px-4 py-4 flex flex-col gap-2">
              ...
              <MobileNavLink label="About" onClick={() => navigate("/about")} />
              ...
              <div className="border-t border-[#3B82F6]/20 mt-2 pt-3 flex flex-col gap-2">
                {isAuthenticated ? (
                  ...
                  <>
                    <button onClick={() => navigate("/login")} className="py-2.5 rounded-xl text-center text-[#E5E7EB] text-sm font-medium">Login</button>
                    <button onClick={() => navigate("/signup")} className="py-2.5 rounded-xl text-center bg-gradient-to-r from-[#3B82F6] to-[#1A2B3E] text-white text-sm font-semibold">Sign Up</button>
                  </>
                )}
              </div>
            </div>
...
const NavLink = ({ onClick, label, icon }) => (
  <motion.button
    onClick={onClick}
    className="relative px-4 py-2 text-[#9CA3AF] hover:text-[#3B82F6] text-sm font-medium rounded-lg hover:bg-[#3B82F6]/10 transition-all duration-200 group flex items-center"
    whileHover={{ scale: 1.02 }}
    whileTap={{ scale: 0.98 }}
  >
    {icon}
